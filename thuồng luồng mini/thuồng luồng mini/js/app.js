...
function loadFavoritesFromStorage() {
    try {
        const data = JSON.parse(localStorage.getItem('tlm_favorites') || '{"Mặc định": []}');
        // Migration: If data is an array (old version), wrap it
        if (Array.isArray(data)) {
            return { "Mặc định": data };
        }
        return data;
    } catch {
        return { "Mặc định": [] };
...
const state = {
    currentPage: 'home',
    favorites: loadFavoritesFromStorage(), // Now an object: { "CollectionName": [id1, id2] }
    searchQuery: '',
    scrollPositions: {}
};
...
// Helper to check if a place is favorited in ANY collection
function isPlaceFavorited(placeId) {
    const target = normalizeFavId(placeId);
    for (const collection in state.favorites) {
        if (state.favorites[collection].some(id => normalizeFavId(id) === target)) {
            return true;
        }
    }
    return false;
...
window.tlmApplyFavorites = function(data) {
    // data can be an object or an array from old cloud backup
    if (Array.isArray(data)) {
        state.favorites = { "Mặc định": data };
    } else {
        state.favorites = data || { "Mặc định": [] };
    }
    updateFavCount();
    document.querySelectorAll('[data-place-id]').forEach(card => {
        ...
        if (btn) btn.classList.toggle('active', isPlaceFavorited(pid));
    });
};
...
function parseRoute(path) {
    ...
    if (route === '/tao-lo-trinh') return { page: 'itineraryGenerator', params };
    ...
    try {
        switch (parsed.page) {
            ...
            case 'itineraryGenerator': 
                import('./pages/itinerary-generator.js').then(m => {
                    mainContent.innerHTML = m.renderItineraryGenerator();
                    m.initItineraryGenerator?.();
                });
                break;
...
function renderFavoritesPage() {
    let html = `
        <div class="page-container" style="background: var(--color-bg-white); min-height: 100vh;">
            <div class="container" style="padding-top: var(--space-8);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-8);">
                    <div>
                        <h1 style="font-size: var(--text-3xl); font-weight: var(--weight-black); color: var(--color-text-heading); margin-bottom: var(--space-2);">Bộ sưu tập của bạn 📁</h1>
                        <p style="color: var(--color-text-secondary);">Quản lý các địa điểm đã lưu theo từng danh mục riêng</p>
                    </div>
                </div>
    `;

    const collections = Object.keys(state.favorites || {});
    
    if (collections.length === 0 || collections.every(c => state.favorites[c].length === 0)) {
        html += `
                <div class="empty-state">
                    <div class="empty-state__icon">💛</div>
                    <h3 class="empty-state__title">Chưa có bộ sưu tập nào</h3>
                    <p class="empty-state__text">Nhấn vào biểu tượng trái tim trên các địa điểm để lưu và tạo bộ sưu tập nhé!</p>
        ...
        for (const collection of collections) {
            const placeIds = state.favorites[collection];
            if (!placeIds || placeIds.length === 0) continue;
            
            const places = placeIds.map(id => getPlaceById(id)).filter(Boolean);
            
            html += `
                <div class="collection-group">
                    <h2 style="font-size: var(--text-xl); font-weight: var(--weight-bold); margin-bottom: var(--space-4); display: flex; align-items: center; gap: var(--space-2);">
                        <i data-lucide="folder" style="color: var(--color-primary);"></i> ${collection} 
                        <span class="badge" style="margin-left: 8px;">${places.length} địa điểm</span>
                    </h2>
                    <div class="scroller" style="padding-bottom: var(--space-4);">
                        ${places.map(p => createPlaceCard(p)).join('')}
                    </div>
                </div>
            `;
        }
...
// Global Functions
// ============================================
window.openCollectionModal = function(placeId) {
    ...
    const collections = Object.keys(state.favorites || {});
    if (collections.length === 0) collections.push('Mặc định');

    modal.innerHTML = `
        <div class="modal" style="width: 100%; max-width: 400px; padding: 24px;">
            <button class="modal__close" onclick="document.getElementById('collection-modal').style.display='none'">
                <i data-lucide="x"></i>
            </button>
            <h3 style="margin-bottom: 16px; font-weight: bold; font-size: 1.25rem;">Lưu vào bộ sưu tập</h3>
            
            <div style="max-height: 300px; overflow-y: auto; margin-bottom: 16px; display: flex; flex-direction: column; gap: 8px;">
                ${collections.map(c => `
                    <button class="btn btn--outline" style="justify-content: flex-start; text-align: left; border-color: var(--color-border);" 
                            onclick="window.saveToCollection('${placeId}', '${c}')">
            ...
            <div style="border-top: 1px solid var(--color-border); padding-top: 16px;">
                <div style="display: flex; gap: 8px;">
                    <input type="text" id="new-collection-name" class="search-bar__input" placeholder="Tên bộ sưu tập mới..." style="padding: 12px; font-size: 14px;">
                    <button class="btn btn--primary" onclick="window.createNewCollection('${placeId}')">Tạo</button>
                </div>
            </div>
        </div>
    `;
...
window.createNewCollection = function(placeId) {
    ...
    window.saveToCollection(placeId, name);
};

window.saveToCollection = function(placeId, collectionName) {
    const normalized = normalizeFavId(placeId);
    
    if (!state.favorites[collectionName]) {
        state.favorites[collectionName] = [];
    }
    
    const idx = state.favorites[collectionName].findIndex(id => normalizeFavId(id) === normalized);
    
    if (idx === -1) {
        state.favorites[collectionName].push(normalized);
        showToast('Đã thêm vào ' + collectionName + ' 💛', 'success');
    } else {
        state.favorites[collectionName].splice(idx, 1);
        showToast('Đã bỏ khỏi ' + collectionName, 'info');
    }
    
    document.getElementById('collection-modal').style.display = 'none';
    persistFavorites();
    
    // Update all fav buttons on screen
    document.querySelectorAll('[data-place-id]').forEach(card => {
        const pid = card.dataset.placeId;
        const btn = card.querySelector('.card__fav-btn');
        if (btn) btn.classList.toggle('active', isPlaceFavorited(pid));
...
window.toggleFavorite = function(placeId, btnEl) {
    // If it's already favorited somewhere, maybe we want to remove it or show modal. 
    // Usually Pinterest shows modal on every click. Let's just show modal.
    window.openCollectionModal(placeId);
};

window.isFavorite = function(placeId) { return isPlaceFavorited(placeId); };
window.showToast = showToast;

function updateFavCount() {
    let count = 0;
    for (const c in state.favorites) {
        count += state.favorites[c].length;
    }
